...
void loop() {
  ...
  toggleSwtich();
  if (ackflag == false) {
    gasConcentration(0, gas[0]); // gasConcentration(gasType,gasConcentration)
    if (alarmFlag1 == true) {
      priorityCheck = true;
    }
    if (alarmFlag2 == false) {
      gasConcentration(1, gas[1]);
      priority();
      if (alarmFlag2 == false && priorityCheck == true) {
        alarmFlag1 = true;
      }
      if (alarmFlag1 == false && alarmFlag2 == false) {
        gasConcentration(2, gas[2]);
        priority();
      }
    }
    alarm();
...
void gasConcentration(int gasType, int gasCon) { //info about gasType and gas concentration
  ...
  */
  if (gasType == 0)       {
    setAlarm(gasCon, 10, 20); //explosive
    gas[0] = gasCon;
  }
  else if (gasType == 1)  {
    setAlarmO2(gasCon, 19, 23); //O2
    gas[1] = gasCon;
  }
  else if (gasType == 2)  {
    setAlarm(gasCon, 20, 100); // CO
    gas[2] = gasCon;
  }
...
void alarm() {
  ...
  if (currentMillis - startMillis >= period)
  {
    if (alarmFlag1 == true || alarmFlag2 == true) {
      ...
      if (alarmFlag1 == true && alarmFlag2 == false ) {
        tone(BUZZER, 261, 250 );
      }
      if (alarmFlag2 == true && alarmFlag1 == false) {
        tone(BUZZER, 440, 250);
      }
